...
output.file.name.p1 <- "distance_vector_stats_all_channels_4thFl-North-Window-mw.csv.txt"
...
# 	                avg_max_10=mean(subset(rssi, rssi>=quantile(rssi, 0.90))),
	  ...
	  write.table(p_stat1, file=output.file.name.p1, sep=",", append=TRUE, col.names=(i==1), row.names=FALSE)
	  rm(p_stat1)
  }
