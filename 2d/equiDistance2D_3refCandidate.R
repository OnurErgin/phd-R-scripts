...
getFirstColumnRefs <- function (refnodes, packets, numRows) {
  ...
  for (ri in 1:(numRows-2)){
    ...
    thirdCandidate <- mostFreqReceiver(valueSet=winners,tIndex=3)
    if (is.null(thirdCandidate)) {
      candidateSet <- c(firstCandidate,secondCandidate)
    }
    else candidateSet <- c(firstCandidate,secondCandidate, thirdCandidate)
    
    if(refnodes[2] %in% candidateSet){ # Arrived at the second reference node; with for loop this must never be TRUE
    ...
    winners <- getSendersWinningReceivers(allPackets=packets,theSender=refnodes[2],excludeList=NULL,includeList=candidateSet)
